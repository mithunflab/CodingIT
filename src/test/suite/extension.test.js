...
describe("Extension Tests", function () {
	...
	it("should open sidebar view", async () => {
		...
		await vscode.commands.executeCommand("CodinIT.plusButtonClicked")
	...
	it("should handle basic commands", async () => {
		...
		await vscode.commands.executeCommand("CodinIT.historyButtonClicked")
		// Success if no error thrown
	})

	it("should handle advanced settings configuration", async () => {
		// Test browser session setting
		await vscode.workspace.getConfiguration().update("CodinIT.disableBrowserTool", true, true)
		const updatedConfig = vscode.workspace.getConfiguration("CodinIT")
		expect(updatedConfig.get("disableBrowserTool")).to.be.true

		// Reset settings
		await vscode.workspace.getConfiguration().update("CodinIT.disableBrowserTool", undefined, true)
	})
})
