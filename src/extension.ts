...
import { createCodinITAPI } from "./exports"
...
export function activate(context: vscode.ExtensionContext) {
	outputChannel = vscode.window.createOutputChannel("CodinIT")
	context.subscriptions.push(outputChannel)

	ErrorService.initialize()
	Logger.initialize(outputChannel)
	Logger.log("CodinIT extension activated")
	...
	vscode.commands.executeCommand("setContext", "CodinIT.isDevMode", IS_DEV && IS_DEV === "true")
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("CodinIT.plusButtonClicked", async (webview: any) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("CodinIT.mcpButtonClicked", (webview: any) => {
	...
	const openCodinITInNewTab = async () => {
		Logger.log("Opening CodinIT in new tab")
		...
		const panel = vscode.window.createWebviewPanel(WebviewProvider.tabPanelId, "CodinIT", targetCol, {
			enableScripts: true,
	...
	context.subscriptions.push(vscode.commands.registerCommand("CodinIT.popoutButtonClicked", openCodinITInNewTab))
	context.subscriptions.push(vscode.commands.registerCommand("CodinIT.openInNewTab", openCodinITInNewTab))

	context.subscriptions.push(
		vscode.commands.registerCommand("CodinIT.settingsButtonClicked", (webview: any) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("CodinIT.historyButtonClicked", (webview: any) => {
			WebviewProvider.getAllInstances().forEach((instance) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("CodinIT.accountButtonClicked", (webview: any) => {
			WebviewProvider.getAllInstances().forEach((instance) => {
	...
	*/
	...
	if (IS_DEV && IS_DEV === "true") {
		...
		import("./dev/commands/tasks")
			.then((module) => {
				...
				Logger.log("CodinIT dev task commands registered")
			})
			.catch((error) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("CodinIT.addToChat", async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("CodinIT.addTerminalOutputToChat", async () => {
		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					const addAction = new vscode.CodeAction("Add to CodinIT", vscode.CodeActionKind.QuickFix)
					addAction.command = {
						command: "CodinIT.addToChat",
						title: "Add to CodinIT",
						arguments: [expandedRange, context.diagnostics],
					}

					const fixAction = new vscode.CodeAction("Fix with CodinIT", vscode.CodeActionKind.QuickFix)
					fixAction.command = {
						command: "CodinIT.fixWithCodinIT",
						title: "Fix with CodinIT",
						arguments: [expandedRange, context.diagnostics],
					}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("CodinIT.focusChatInput", () => {
	...
	return createCodinITAPI(outputChannel, sidebarWebview.controller)
}
...
export function deactivate() {
	...
	Logger.log("CodinIT extension deactivated")
}
