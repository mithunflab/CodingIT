...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = CodinITIgnoreController ? CodinITIgnoreController.filterPaths(filesToParse) : filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, CodinITIgnoreController)
		if (definitions) {
...
async function parseFile(
	...
	if (CodinITIgnoreController && !CodinITIgnoreController.validateAccess(filePath)) {
		return null
	}
