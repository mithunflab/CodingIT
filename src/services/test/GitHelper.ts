...
export async function validateWorkspacePath(workspacePath: string): Promise<void> {
	...
	try {
		const testFile = path.join(workspacePath, ".CodinIT_write_test")
...
export async function initializeGitRepository(workspacePath: string): Promise<boolean> {
	...
	try {
		...
		await execa("git", ["config", "user.name", "CodinIT Evaluation"], { cwd: workspacePath })
		await execa("git", ["config", "user.email", "CodinIT@example.com"], { cwd: workspacePath })
