...
import { CodinITAsk, ExtensionMessage } from "@shared/ExtensionMessage"
import { ApiProvider } from "@shared/api"
import { WebviewMessage } from "@shared/WebviewMessage"
import { HistoryItem } from "@shared/HistoryItem"
import { getSavedCodinITMessages, getSavedApiConversationHistory } from "@core/storage/disk"
...
export function createTestServer(webviewProvider?: WebviewProvider): http.Server {
	// Try to show the CodinIT sidebar
	Logger.log("[createTestServer] Opening CodinIT in sidebar...")
		...
		req.on("end", async () => {
			try {
				...
				if (!visibleWebview || !visibleWebview.controller) {
					res.writeHead(500)
					res.end(JSON.stringify({ error: "No active CodinIT instance found" }))
					return
				}
				...
				try {
					...
					if (apiKey) {
						...
						const updatedConfig = {
							...apiConfiguration,
							apiProvider: "CodinIT" as ApiProvider,
							CodinITApiKey: apiKey,
						}

						// Store the API key securely
						await storeSecret(visibleWebview.controller.context, "CodinITApiKey", apiKey)

						// Update the API configuration
						await updateApiConfiguration(visibleWebview.controller.context, updatedConfig)

						// Update global state to use CodinIT provider
						await updateGlobalState(visibleWebview.controller.context, "apiProvider", "CodinIT" as ApiProvider)
					...
					try {
						...
						try {
							if (typeof taskId === "string") {
								messages = await getSavedCodinITMessages(visibleWebview.controller.context, taskId)
							}
						} catch (error) {
							Logger.log(`Error getting saved CodinIT messages: ${error}`)
						}
...
export function createMessageCatcher(webviewProvider: WebviewProvider): vscode.Disposable {
	Logger.log("CodinIT message catcher registered")
	...
	if (webviewProvider && webviewProvider.controller) {
		...
		webviewProvider.controller.postMessageToWebview = async (message: ExtensionMessage) => {
			...
			if (message.type === "partialMessage" && message.partialMessage?.type === "ask" && !message.partialMessage.partial) {
				const askType = message.partialMessage.ask as CodinITAsk
	...
	return new vscode.Disposable(() => {
		...
		Logger.log("CodinIT message catcher disposed")
	})
}
 ...
 */
function autoRespondToAsk(webviewProvider: WebviewProvider, askType: CodinITAsk, askText?: string): void {
	...
	switch (askType) {
		...
		case "new_task":
			// DeCodinIT creating a new task to keep the current task running
			response.askResponse = "messageResponse"
