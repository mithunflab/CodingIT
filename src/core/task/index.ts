...
import {
	...
	BrowserActionResult,
	browserActions,
	CodinITApiReqCancelReason,
	CodinITApiReqInfo,
	CodinITAsk,
	CodinITAskQuestion,
	CodinITAskUseMcpServer,
	CodinITMessage,
	CodinITPlanModeResponse,
	CodinITSay,
	CodinITSayBrowserAction,
	CodinITSayTool,
...
import { CodinITAskResponse, CodinITCheckpointRestore } from "@shared/WebviewMessage"
...
import {
	...
	getSavedCodinITMessages,
	saveApiConversationHistory,
	saveCodinITMessages,
} from "@core/storage/disk"
import {
	getGlobalCodinITRules,
...
export class Task {
	...
	private CodinITIgnoreController: CodinITIgnoreController
	private askResponse?: CodinITAskResponse
	...
	private async addToCodinITMessages(message: CodinITMessage) {
		// these values allow us to reconstruct the conversation history at the time this CodinIT message was created
		// it's important that apiConversationHistory is initialized before we add CodinIT messages
		message.conversationHistoryIndex = this.apiConversationHistory.length - 1 // NOTE: this is the index of the last added message which is the user message, and once the CodinITmessages have been presented we update the apiconversationhistory with the completed assistant message. This means when resetting to a message, we need to +1 this index to get the correct assistant message that this tool use corresponds to
		message.conversationHistoryDeletedRange = this.conversationHistoryDeletedRange
		this.CodinITMessages.push(message)
		await this.saveCodinITMessagesAndUpdateHistory()
	}

	private async overwriteCodinITMessages(newMessages: CodinITMessage[]) {
		this.CodinITMessages = newMessages
		await this.saveCodinITMessagesAndUpdateHistory()
	}

	private async saveCodinITMessagesAndUpdateHistory() {
		try {
			await saveCodinITMessages(this.getContext(), this.taskId, this.CodinITMessages)

			// combined as they are in ChatView
			const apiMetrics = getApiMetrics(combineApiRequests(combineCommandSequences(this.CodinITMessages.slice(1))))
			const taskMessage = this.CodinITMessages[0] // first message is always the task say
			const lastRelevantMessage =
				this.CodinITMessages[
					findLastIndex(this.CodinITMessages, (m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"))
				]
			...
			console.error("Failed to save CodinIT messages:", error)
		}
	}
	...
	async restoreCheckpoint(messageTs: number, restoreType: CodinITCheckpointRestore, offset?: number) {
		...
		if (!message) {
			console.error("Message not found", this.CodinITMessages)
			return
		}
		...
		if (!didWorkspaceRestoreFail) {
			switch (restoreType) {
				...
				case "taskAndWorkspace":
					...
					const deletedMessages = this.CodinITMessages.slice(messageIndex + 1)
					const deletedApiReqsMetrics = getApiMetrics(combineApiRequests(combineCommandSequences(deletedMessages)))

					const newCodinITMessages = this.CodinITMessages.slice(0, messageIndex + 1)
					await this.overwriteCodinITMessages(newCodinITMessages) // calls saveCodinITMessages which saves historyItem
			...
			if (restoreType !== "task") {
				...
				const checkpointMessages = this.CodinITMessages.filter((m) => m.say === "checkpoint_created")
	...
	async presentMultifileDiff(messageTs: number, seeNewChangesSinceLastTaskCompletion: boolean) {
		...
		const messageIndex = this.CodinITMessages.findIndex((m) => m.ts === messageTs)
		const message = this.CodinITMessages[messageIndex]
		if (!message) {
		...
		try {
			if (seeNewChangesSinceLastTaskCompletion) {
				...
				const firstCheckpointMessageCheckpointHash = this.CodinITMessages.find(
	...
	async doesLatestTaskCompletionHaveNewChanges() {
		const messageIndex = findLastIndex(this.CodinITMessages, (m) => m.say === "completion_result")
		const message = this.CodinITMessages[messageIndex]
		if (!message) {
	...
	async ask(
		...
		response: CodinITAskResponse
		text?: string
		images?: string[]
	}> {
		// If this CodinIT instance was aborted by the provider, then the only thing keeping us alive is a promise still running in the background, in which case we don't want to send its result to the webview as it is attached to a new instance of CodinIT now. So we can safely ignore the result of any active promises, and this class will be deallocated. (Although we set CodinIT = undefined in provider, that simply removes the reference to this instance, but the instance is still alive until this promise resolves or rejects.)
		if (this.abort) {
			throw new Error("CodinIT instance aborted")
		}
		let askTs: number
		if (partial !== undefined) {
			const lastMessage = this.CodinITMessages.at(-1)
			...
			if (partial) {
				if (isUpdatingPreviousPartial) {
					...
					await this.addToCodinITMessages({
						ts: askTs,
						type: "ask",
						ask: type,
						text,
	...
	async handleWebviewAskResponse(askResponse: CodinITAskResponse, text?: string, images?: string[]) {
		this.askResponse = askResponse
		this.askResponseText = text
		this.askResponseImages = images
	}

	async say(type: CodinITSay, text?: string, images?: string[], partial?: boolean): Promise<undefined> {
		if (this.abort) {
			throw new Error("CodinIT instance aborted")
		}
	...
	async removeLastPartialMessageIfExistsWithType(type: "ask" | "say", askOrSay: CodinITAsk | CodinITSay) {
		const lastMessage = this.CodinITMessages.at(-1)
		if (lastMessage?.partial && lastMessage.type === type && (lastMessage.ask === askOrSay || lastMessage.say === askOrSay)) {
			this.CodinITMessages.pop()
			await this.saveCodinITMessagesAndUpdateHistory()
			await this.postStateToWebview()
		}
	}
	...
	private async startTask(task?: string, images?: string[]): Promise<void> {
		// conversationHistory (for API) and CodinITMessages (for webview) need to be in sync
		// if the extension process were killed, then on restart the CodinITMessages might not be empty, so we need to set it to [] when we create a new CodinIT client (otherwise webview would show stale messages from previous session)
		this.CodinITMessages = []
		this.apiConversationHistory = []
	...
	private async resumeTaskFromHistory() {
		...
		const modifiedCodinITMessages = await getSavedCodinITMessages(this.getContext(), this.taskId)

		// Remove any resume messages that may have been added before
		const lastRelevantMessageIndex = findLastIndex(
			modifiedCodinITMessages,
			(m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"),
		)
		if (lastRelevantMessageIndex !== -1) {
			modifiedCodinITMessages.splice(lastRelevantMessageIndex + 1)
		}
		...
		if (lastApiReqStartedIndex !== -1) {
			const lastApiReqStarted = modifiedCodinITMessages[lastApiReqStartedIndex]
			const { cost, cancelReason }: CodinITApiReqInfo = JSON.parse(lastApiReqStarted.text || "{}")
			if (cost === undefined && cancelReason === undefined) {
				modifiedCodinITMessages.splice(lastApiReqStartedIndex, 1)
			}
		}

		await this.overwriteCodinITMessages(modifiedCodinITMessages)
		this.CodinITMessages = await getSavedCodinITMessages(this.getContext(), this.taskId)

		// Now present the CodinIT messages to the user and ask if they want to resume (NOTE: we ran into a bug before where the apiconversationhistory wouldn't be initialized when opening a old task, and it was because we were waiting for resume)
		...
		let askType: CodinITAsk
		if (lastCodinITMessage?.ask === "completion_result") {
		...
		// need to make sure that the api conversation history can be resumed by the api, even if it goes out of sync with CodinIT messages
		...
		const agoText = (() => {
			const timestamp = lastCodinITMessage?.ts ?? Date.now()
			const now = Date.now()
		...
		const wasRecent = lastCodinITMessage?.ts && Date.now() - lastCodinITMessage.ts < 30_000
	...
	private async initiateTaskLoop(userContent: UserContent): Promise<void> {
		...
		while (!this.abort) {
			const didEndLoop = await this.recursivelyMakeCodinITRequests(nextUserContent, includeFileDetails)
			includeFileDetails = false // we only need file details the first time

			//  The way this agentic loop works is that CodinIT will be given a task that he then calls tools to complete. unless there's an attempt_completion call, we keep responding back to him with his tool's responses until he either attempt_completion or does not use anymore tools. If he does not use anymore tools, we ask him to consider if he's completed the task and then call attempt_completion, otherwise proceed with completing the task.
			// There is a MAX_REQUESTS_PER_TASK limit to prevent infinite requests, but CodinIT is prompted to finish the task as efficiently as he can.
			...
			if (didEndLoop) {
				...
				// this.say(
				// 	"tool",
				// 	"CodinIT responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
				// )
				nextUserContent = [
					{
						type: "text",
	...
	async abortTask() {
		...
		this.CodinITIgnoreController.dispose()
	...
	async saveCheckpoint(isAttemptCompletionMessage: boolean = false) {
		...
		this.CodinITMessages.forEach((message) => {
		...
		if (!isAttemptCompletionMessage) {
			...
			this.checkpointTracker?.commit().then(async (commitHash) => {
				const lastCheckpointMessage = findLast(this.CodinITMessages, (m) => m.say === "checkpoint_created")
		...
		// for (let i = this.CodinITMessages.length - 1; i >= 0; i--) {
		// 	const message = this.CodinITMessages[i]
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const disableBrowserTool = vscode.workspace.getConfiguration("CodinIT").get<boolean>("disableBrowserTool") ?? false
		// CodinIT browser tool uses image recognition for navigation (requires model image support).
		...
		const preferredLanguage = getLanguageKey(
			vscode.workspace.getConfiguration("CodinIT").get<LanguageDisplay>("preferredLanguage"),
		)
		...
		const { globalToggles, localToggles } = await refreshCodinITRulesToggles(this.getContext(), cwd)

		const globalCodinITRulesFilePath = await ensureRulesDirectoryExists()
		const globalCodinITRulesFileInstructions = await getGlobalCodinITRules(globalCodinITRulesFilePath, globalToggles)

		const localCodinITRulesFileInstructions = await getLocalCodinITRules(cwd, localToggles)

		const CodinITIgnoreContent = this.CodinITIgnoreController.CodinITIgnoreContent
		let CodinITIgnoreInstructions: string | undefined
		if (CodinITIgnoreContent) {
			CodinITIgnoreInstructions = formatResponse.CodinITIgnoreInstructions(CodinITIgnoreContent)
		}

		if (
			settingsCustomInstructions ||
			globalCodinITRulesFileInstructions ||
			localCodinITRulesFileInstructions ||
			CodinITIgnoreInstructions ||
		...
		if (contextManagementMetadata.updatedConversationHistoryDeletedRange) {
			...
			await this.saveCodinITMessagesAndUpdateHistory() // saves task history item which we use to keep track of conversation history deleted range
		}
	...
	async presentAssistantMessage() {
		...
		switch (block.type) {
			...
			case "tool_use":
				...
				const askApproval = async (type: CodinITAsk, partialMessage?: string) => {
				...
				switch (block.name) {
					...
					case "replace_in_file": {
						...
						const accessAllowed = this.CodinITIgnoreController.validateAccess(relPath)
						if (!accessAllowed) {
							await this.say("CodinITignore_error", relPath)
							pushToolResult(formatResponse.toolError(formatResponse.CodinITIgnoreError(relPath)))
						...
						try {
							...
							if (diff) {
								...
								// open the editor if not done already.  This is to fix diff error when model provides correct search-replace text but CodinIT throws error
							...
							const sharedMessageProps: CodinITSayTool = {
							...
							if (block.partial) {
								...
								if (this.shouldAutoApproveToolWithPath(block.name, relPath)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`CodinIT wants to ${fileExists ? "edit" : "create"} ${path.basename(relPath)}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
								...
								// Mark the file as edited by CodinIT to prevent false "recently modified" warnings
								this.fileContextTracker.markFileAsEditedByCodinIT(relPath)
								...
								await this.fileContextTracker.trackFileContext(relPath, "CodinIT_edited")
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`CodinIT wants to read ${path.basename(absolutePath)}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`CodinIT wants to view directory ${path.basename(absolutePath)}/`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
									const didApprove = await askApproval("tool", completeMessage)
									if (!didApprove) {
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`CodinIT wants to view source code definitions in ${path.basename(absolutePath)}/`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
									const didApprove = await askApproval("tool", completeMessage)
									if (!didApprove) {
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`CodinIT wants to search files in ${path.basename(absolutePath)}/`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
									const didApprove = await askApproval("tool", completeMessage)
									if (!didApprove) {
							...
							if (block.partial) {
								if (action === "launch") {
									...
									await this.say(
										...
										} satisfies CodinITSayBrowserAction),
										undefined,
										block.partial,
									)
								}
								...
								if (action === "launch") {
									...
									if (this.shouldAutoApproveTool(block.name)) {
										...
										showNotificationForApprovalIfAutoApprovalEnabled(
											`CodinIT wants to use a browser and launch ${url}`,
										)
										this.removeLastPartialMessageIfExistsWithType("say", "browser_action_launch")
					...
					case "execute_command": {
						...
						try {
							if (block.partial) {
								...
								const ignoredFileAttemptedToAccess = this.CodinITIgnoreController.validateCommand(command)
								if (ignoredFileAttemptedToAccess) {
									await this.say("CodinITignore_error", ignoredFileAttemptedToAccess)
									pushToolResult(
										formatResponse.toolError(formatResponse.CodinITIgnoreError(ignoredFileAttemptedToAccess)),
									)
								...
								if (
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`CodinIT wants to execute a command: ${command}`,
									)
					...
					case "use_mcp_tool": {
						...
						try {
							if (block.partial) {
								...
								} satisfies CodinITAskUseMcpServer)
								...
								if (mcp_arguments) {
									try {
										...
										await this.say(
											"error",
											`CodinIT tried to use ${tool_name} with an invalid JSON argument. Retrying...`,
										)
										pushToolResult(
											formatResponse.toolError(
								...
								if (this.shouldAutoApproveTool(block.name) && isToolAutoApproved) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`CodinIT wants to use ${tool_name} on ${server_name}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server")
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`CodinIT wants to access ${uri} on ${server_name}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server")
									const didApprove = await askApproval("use_mcp_server", completeMessage)
									if (!didApprove) {
					...
					case "ask_followup_question": {
						...
						try {
							if (block.partial) {
								...
								if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
									showSystemNotification({
										subtitle: "CodinIT has a question...",
								...
								if (optionsRaw && text && parsePartialArrayString(optionsRaw).includes(text)) {
									...
									const lastFollowupMessage = findLast(this.CodinITMessages, (m) => m.ask === "followup")
					...
					case "new_task": {
						...
						try {
							if (block.partial) {
								...
								if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
									showSystemNotification({
										subtitle: "CodinIT wants to start a new task...",
										message: `CodinIT is suggesting to start a new task with: ${context}`,
									})
								}
					...
					case "condense": {
						...
						try {
							if (block.partial) {
								...
								if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
									showSystemNotification({
										subtitle: "CodinIT wants to condense the conversation...",
										message: `CodinIT is suggesting to condense your conversation with: ${context}`,
									})
								}
					...
					case "plan_mode_respond": {
						...
						const sharedMessage = {
							...
							options: parsePartialArrayString(removeClosingTag("options", optionsRaw)),
						} satisfies CodinITPlanModeResponse
						try {
							if (block.partial) {
								...
								// if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
								// 	showSystemNotification({
								// 		subtitle: "CodinIT has a response...",
								...
								if (optionsRaw && text && parsePartialArrayString(optionsRaw).includes(text)) {
									// Valid option selected, don't show user message in UI
									// Update last followup message with selected option
									const lastPlanMessage = findLast(this.CodinITMessages, (m) => m.ask === "plan_mode_respond")
									if (lastPlanMessage) {
					...
					case "attempt_completion": {
						...
						if (command) {
							...
							// TODO: currently we don't handle if this command fails, it could be useful to let CodinIT know and retry
						...
						try {
							...
							if (block.partial) {
								if (command) {
									...
									// const secondLastMessage = this.CodinITMessages.at(-2)
	...
	async recursivelyMakeCodinITRequests(userContent: UserContent, includeFileDetails: boolean = false): Promise<boolean> {
		if (this.abort) {
			throw new Error("CodinIT instance aborted")
		}
		...
		if (this.consecutiveMistakeCount >= 3) {
			if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					subtitle: "Error",
					message: "CodinIT is having trouble. Would you like to continue the task?",
				})
			}
			const { response, text, images } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "CodinIT uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 3.7 Sonnet for its advanced agentic coding capabilities.",
			)
			if (response === "messageResponse") {
				userContent.push(
					...[
		...
		if (
			...
			if (this.autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					...
					message: `CodinIT has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests.`,
				})
			}
			await this.ask(
				"auto_approval_max_req_reached",
				`CodinIT has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests. Would you like to reset the count and proceed with the task?`,
			)
		...
		const previousApiReqIndex = findLastIndex(this.CodinITMessages, (m) => m.say === "api_req_started")

		// Save checkpoint if this is the first API request
		const isFirstRequest = this.CodinITMessages.filter((m) => m.say === "api_req_started").length === 0
		if (isFirstRequest) {
			...
			try {
				this.checkpointTracker = await pTimeout(
					CheckpointTracker.create(this.taskId, this.context.globalStorageUri.fsPath),
					{
						milliseconds: 15_000,
						message:
							"Checkpoints taking too long to initialize. Consider re-opening CodinIT in a project that uses git, or disabling checkpoints.",
					},
				)
			} catch (error) {
				const errorMessage = error instanceof Error ? error.message : "Unknown error"
				console.error("Failed to initialize checkpoint tracker:", errorMessage)
				this.checkpointTrackerErrorMessage = errorMessage // will be displayed right away since we saveCodinITMessages next which posts state to webview
			}
		}
		...
		const [parsedUserContent, environmentDetails, CodinITrulesError] = await this.loadContext(userContent, includeFileDetails)

		// error handling if the user uses the /newrule command & their .CodinITrules is a file, for file read operations didnt work properly
		if (CodinITrulesError === true) {
			await this.say(
				"error",
				"Issue with processing the /newrule command. Double check that, if '.CodinITrules' already exists, it's a directory and not a file. Otherwise there was an issue referencing this file/directory.",
			)
		}
		...
		const lastApiReqIndex = findLastIndex(this.CodinITMessages, (m) => m.say === "api_req_started")
		this.CodinITMessages[lastApiReqIndex].text = JSON.stringify({
			request: userContent.map((block) => formatContentBlockToMarkdown(block)).join("\n\n"),
		} satisfies CodinITApiReqInfo)
		await this.saveCodinITMessagesAndUpdateHistory()
		await this.postStateToWebview()
		...
		try {
			...
			const updateApiReqMsg = (cancelReason?: CodinITApiReqCancelReason, streamingFailedMessage?: string) => {
				this.CodinITMessages[lastApiReqIndex].text = JSON.stringify({
					...JSON.parse(this.CodinITMessages[lastApiReqIndex].text || "{}"),
			...
			const abortStream = async (cancelReason: CodinITApiReqCancelReason, streamingFailedMessage?: string) => {
				if (this.diffViewProvider.isEditing) {
			...
			try {
				for await (const chunk of stream) {
					...
					if (this.abort) {
						...
						if (!this.abandoned) {
							// only need to gracefully abort if this instance isn't abandoned (sometimes openrouter stream hangs, in which case this would affect future instances of CodinIT)
				...
				// abandoned happens when extension is no longer waiting for the CodinIT instance to finish aborting (error is thrown here when any function in the for loop throws due to this.abort)
				if (!this.abandoned) {
			...
			// OpenRouter/CodinIT may not return token usage as part of the stream (since it may abort early), so we fetch after the stream is finished
			...
			if (assistantMessage.length > 0) {
				...
				const recDidEndLoop = await this.recursivelyMakeCodinITRequests(this.userMessageContent)
	...
	async loadContext(userContent: UserContent, includeFileDetails: boolean = false): Promise<[UserContent, string, boolean]> {
		// Track if we need to check CodinITrulesFile
		let needsCodinITrulesFileCheck = false
				...
				userContent.map(async (block) => {
					if (block.type === "text") {
						...
						if (
							...
							const { processedText, needsCodinITrulesFileCheck: needsCheck } = parseSlashCommands(parsedText)

							if (needsCheck) {
								needsCodinITrulesFileCheck = true
							}
		...
		// After processing content, check CodinITrulesData if needed
		let CodinITrulesError = false
		if (needsCodinITrulesFileCheck) {
			CodinITrulesError = await ensureLocalCodinITrulesDirExists(cwd)
		}

		// Return all results
		return [processedUserContent, environmentDetails, CodinITrulesError]
	}

	async getEnvironmentDetails(includeFileDetails: boolean = false) {
		let details = ""

		// It could be useful for CodinIT to know if the user went from one or no file to another between messages, so we always include this context
		...
		const diagnostics = await this.diagnosticsMonitor.getCurrentDiagnostics(this.didEditFile || terminalWasBusy) // if CodinIT ran a command (ie npm install) or edited the workspace then wait a bit for updated diagnostics
		...
		*/
		...
		if (includeFileDetails) {
			...
			if (isDesktop) {
				...
				const result = formatResponse.formatFilesList(cwd, files, didHitLimit, this.CodinITIgnoreController)
				details += result
			}
		}
		...
		const getTotalTokensFromApiReqMessage = (msg: CodinITMessage) => {
			if (!msg.text) {
				return 0
			}
			try {
		...
		const modifiedMessages = combineApiRequests(combineCommandSequences(this.CodinITMessages.slice(1)))
