import { CodinITIgnoreController } from "./CodinITIgnoreController"
...
describe("CodinITIgnoreController", () => {
	let tempDir: string
	let controller: CodinITIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .CodinITignore file
		await fs.writeFile(
			path.join(tempDir, ".CodinITignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new CodinITIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .CodinITignore file", async () => {
			const result = controller.validateAccess(".CodinITignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .CodinITignore", async () => {
			// Create a new .CodinITignore with comments
			await fs.writeFile(
				path.join(tempDir, ".CodinITignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .CodinITignore gracefully", async () => {
			// Create a new controller in a directory without .CodinITignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new CodinITIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .CodinITignore", async () => {
			await fs.writeFile(path.join(tempDir, ".CodinITignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .CodinITignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".CodinITignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .CodinITignore
			controller = new CodinITIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .CodinITignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .CodinITignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".CodinITignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".CodinITignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
