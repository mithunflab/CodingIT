...
import { CodinITCheckpointRestore, WebviewMessage } from "@shared/WebviewMessage"
...
export class Controller {
	...
	mcpHub: McpHub
	accountService: CodinITAccountService
	...
	constructor(
		...
		this.outputChannel.appendLine("CodinITProvider instantiated")
		...
		this.accountService = new CodinITAccountService(
			(msg) => this.postMessageToWebview(msg),
			async () => {
				const { apiConfiguration } = await this.getStateToPostToWebview()
				return apiConfiguration?.CodinITApiKey
			},
		)
	...
	*/
	async dispose() {
		this.outputChannel.appendLine("Disposing CodinITProvider...")
	...
	async handleSignOut() {
		try {
			await storeSecret(this.context, "CodinITApiKey", undefined)
			await updateGlobalState(this.context, "userInfo", undefined)
			await updateGlobalState(this.context, "apiProvider", "openrouter")
			await this.postStateToWebview()
			vscode.window.showInformationMessage("Successfully logged out of CodinIT")
		} catch (error) {
	...
	async handleWebviewMessage(message: WebviewMessage) {
		switch (message.type) {
			...
			case "newTask":
				...
				// initializing new instance of CodinIT will make sure that any agentically running promises in old instance don't affect our new task. this essentially creates a fresh slate for the new task
			...
			case "refreshOpenAiModels":
				...
				break
			case "refreshCodinITRules":
				await refreshCodinITRulesToggles(this.context, cwd)
				await this.postStateToWebview()
				break
			case "openInBrowser":
				if (message.url) {
			...
			// 	if (message.text) {
			// 		const response = await fetch(`https://api.CodinIT.bot/v1/mcp/marketplace/item?mcpId=${message.mcpId}`)
			...
			case "toggleCodinITRule": {
				const { isGlobal, rulePath, enabled } = message
				if (rulePath && typeof enabled === "boolean" && typeof isGlobal === "boolean") {
					if (isGlobal) {
						const toggles =
							((await getGlobalState(this.context, "globalCodinITRulesToggles")) as CodinITRulesToggles) || {}
						toggles[rulePath] = enabled
						await updateGlobalState(this.context, "globalCodinITRulesToggles", toggles)
					} else {
						const toggles =
							((await getWorkspaceState(this.context, "localCodinITRulesToggles")) as CodinITRulesToggles) || {}
						toggles[rulePath] = enabled
						await updateWorkspaceState(this.context, "localCodinITRulesToggles", toggles)
					}
					await this.postStateToWebview()
				} else {
					console.error("toggleCodinITRule: Missing or invalid parameters", {
						rulePath,
			...
			case "deleteCodinITRule": {
				...
				if (rulePath && typeof isGlobal === "boolean") {
					...
					console.error("deleteCodinITRule: Missing or invalid parameters", {
						rulePath,
						isGlobal: typeof isGlobal === "boolean" ? isGlobal : `Invalid: ${typeof isGlobal}`,
					})
				}
	...
	async cancelTask() {
		if (this.task) {
			...
			if (this.task) {
				// 'abandoned' will prevent this CodinIT instance from affecting future CodinIT instance gui. this may happen if its hanging on a streaming request
				this.task.abandoned = true
			}
			await this.initTask(undefined, undefined, historyItem) // clears task again, so we need to abortTask manually above
			// await this.postStateToWebview() // new CodinIT instance will post state when it's ready. having this here sent an empty messages array to webview leading to virtuoso having to reload the entire list
		}
	}
	...
	async handleAuthCallback(customToken: string, apiKey: string) {
		try {
			...
			await storeSecret(this.context, "CodinITApiKey", apiKey)
			...
			const updatedConfig = {
				...apiConfiguration,
				apiProvider: CodinITProvider,
				CodinITApiKey: apiKey,
			}
			...
			await this.postStateToWebview()
			// vscode.window.showInformationMessage("Successfully logged in to CodinIT")
		} catch (error) {
			console.error("Failed to handle auth callback:", error)
			vscode.window.showErrorMessage("Failed to log in to CodinIT")
	...
	private async fetchMcpMarketplaceFromApi(silent: boolean = false): Promise<McpMarketplaceCatalog | undefined> {
		try {
			const response = await axios.get("https://api.CodinIT.bot/v1/mcp/marketplace", {
				headers: {
	...
	private async downloadMcp(mcpId: string) {
		try {
			...
			const response = await axios.post<McpDownloadResponse>(
				"https://api.CodinIT.bot/v1/mcp/download",
				{ mcpId },
				{
...
- Use "${mcpDetails.mcpId}" as the server name in CodinIT_mcp_settings.json.
- Create the directory for the new MCP server before starting installation.
- Make sure you read the user's existing CodinIT_mcp_settings.json file before editing it with this new mcp, to not overwrite any existing servers.
...
Here is the project's README to help you get started:\n\n${mcpDetails.readmeContent}\n${mcpDetails.llmsInstallationContent}`
	...
	// 'Add to CodinIT' context menu in editor and code action
	...
	// 'Add to CodinIT' context menu in Terminal
	...
	// 'Fix with CodinIT' in code actions
	async fixWithCodinIT(code: string, filePath: string, languageId: string, diagnostics: vscode.Diagnostic[]) {
		// Ensure the sidebar view is visible
		await vscode.commands.executeCommand("claude-dev.SidebarProvider.focus")
		await setTimeoutPromise(100)
		...
		console.log("fixWithCodinIT", code, filePath, languageId, diagnostics, problemsString)
	}
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		const localCodinITRulesToggles =
			((await getWorkspaceState(this.context, "localCodinITRulesToggles")) as CodinITRulesToggles) || {}
	...
	/*
	Now that we use retainContextWhenHidden, we don't have to store a cache of CodinIT messages in the user's state, but we could to reduce memory footprint in long conversations.

	- We have to be careful of what state is shared between CodinITProvider instances since there could be multiple instances of the extension running at once. For example when we cached CodinIT messages using the same key, two instances of the extension could end up using the same key and overwriting each other's messages.
	- Some state does need to be shared between the instances, i.e. the API key--however there doesn't seem to be a good way to notify the other instances that the API key has changed.

	We need to use a unique identifier for each CodinITProvider instance's message cache since we could be running several instances of the extension outside of just the sidebar i.e. in editor panels.
