 ...
 */
export function parseSlashCommands(text: string): { processedText: string; needsCodinITrulesFileCheck: boolean } {
	...
	for (const { tag, regex } of tagPatterns) {
		...
		if (match) {
			...
			if (SUPPORTED_COMMANDS.includes(commandName)) {
				...
				return { processedText: processedText, needsCodinITrulesFileCheck: commandName === "newrule" ? true : false }
			}
		}
	}

	// if no supported commands are found, return the original text
	return { processedText: text, needsCodinITrulesFileCheck: false }
}
