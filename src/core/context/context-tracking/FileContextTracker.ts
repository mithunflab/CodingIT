...
// If a user modifies a file outside of CodinIT, the context may become stale and need to be updated.
// We do not want CodinIT to reload the context every time a file is modified, so we use this class merely
// to inform CodinIT that the change has occurred, and tell CodinIT to reload the file before making
// any changes to it. This fixes an issue with diff editing, where CodinIT was unable to complete a diff edit.
// a diff edit because the file was modified since CodinIT last read it.

// FileContextTracker
//
// This class is responsible for tracking file operations.
// If the full contents of a file are pass to CodinIT via a tool, mention, or edit, the file is marked as active.
// If a file is modified outside of CodinIT, we detect and track this change to prevent stale context.
export class FileContextTracker {
	...
	private recentlyEditedByCodinIT = new Set<string>()
	...
	async setupFileWatcher(filePath: string) {
		...
		// Track file changes
		watcher.onDidChange(() => {
			if (this.recentlyEditedByCodinIT.has(filePath)) {
				this.recentlyEditedByCodinIT.delete(filePath) // This was an edit by CodinIT, no need to inform CodinIT
			} else {
				this.recentlyModifiedFiles.add(filePath) // This was a user edit, we will inform CodinIT
	...
	// This is the main entry point for FileContextTracker and is called when a file is passed to CodinIT via a tool, mention, or edit.
	async trackFileContext(filePath: string, operation: "read_tool" | "user_edited" | "CodinIT_edited" | "file_mentioned") {
		try {
			const cwd = this.getCwd()
			if (!cwd) {
				return
	...
	async addFileToFileContextTracker(
		...
		try {
			...
			let newEntry: FileMetadataEntry = {
				...
				CodinIT_read_date: getLatestDateForField(filePath, "CodinIT_read_date"),
				CodinIT_edit_date: getLatestDateForField(filePath, "CodinIT_edit_date"),
			...
			switch (source) {
				...
				// CodinIT_edited: CodinIT has edited the file
				case "CodinIT_edited":
					newEntry.CodinIT_read_date = now
					newEntry.CodinIT_edit_date = now
					break

				// read_tool/file_mentioned: CodinIT has read the file via a tool or file mention
				case "read_tool":
				case "file_mentioned":
					newEntry.CodinIT_read_date = now
					break
	...
	// Marks a file as edited by CodinIT to prevent false positives in file watchers
	markFileAsEditedByCodinIT(filePath: string): void {
		this.recentlyEditedByCodinIT.add(filePath)
	}
