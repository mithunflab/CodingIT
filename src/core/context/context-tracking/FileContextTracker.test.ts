...
describe("FileContextTracker", () => {
	...
	it("should add a record when a file is read by a tool", async () => {
		...
		expect(fileEntry.CodinIT_read_date).to.be.a("number")
		expect(fileEntry.CodinIT_edit_date).to.be.null
	})

	it("should add a record when a file is edited by CodinIT", async () => {
		const filePath = "src/test-file.ts"

		await tracker.trackFileContext(filePath, "CodinIT_edited")
		...
		expect(activeEntry.record_source).to.equal("CodinIT_edited")
		expect(activeEntry.CodinIT_read_date).to.be.a("number")
		expect(activeEntry.CodinIT_edit_date).to.be.a("number")
	})
	...
	it("should mark existing entries as stale when adding a new entry for the same file", async () => {
		...
		mockTaskMetadata.files_in_context = [
			{
				...
				CodinIT_read_date: Date.now() - 1000, // 1 second ago
				CodinIT_edit_date: null,
				user_edit_date: null,
			},
		]
		...
		expect(newEntry.record_source).to.equal("CodinIT_edited")
	})
	...
	it("should not track CodinIT edits as user edits", async () => {
		const filePath = "src/test-file.ts"

		// First track the file to set up the watcher
		await tracker.trackFileContext(filePath, "read_tool")

		// Mark the file as edited by CodinIT
		tracker.markFileAsEditedByCodinIT(filePath)
