...
import { CodinITApiReqInfo, CodinITMessage } from "@shared/ExtensionMessage"
...
class ContextManager {
	getNewContextMessagesAndMetadata(
		...
		if (previousApiReqIndex >= 0) {
			const previousRequest = CodinITMessages[previousApiReqIndex]
			if (previousRequest && previousRequest.text) {
				const { tokensIn, tokensOut, cacheWrites, cacheReads }: CodinITApiReqInfo = JSON.parse(previousRequest.text)
	...
	public getNextTruncationRange(
		...
		// NOTE: anthropic format messages are always user-assistant-user-assistant, while openai format messages can have multiple user messages in a row (we use anthropic format throughout CodinIT)
