...
import { CodinITMessage } from "@shared/ExtensionMessage"
...
export const GlobalFileNames = {
	...
	mcpSettings: "CodinIT_mcp_settings.json",
	CodinITRules: ".CodinITrules",
...
export async function ensureRulesDirectoryExists(): Promise<string> {
	...
	const CodinITRulesDir = path.join(userDocumentsPath, "CodinIT", "Rules")
	try {
		await fs.mkdir(CodinITRulesDir, { recursive: true })
	} catch (error) {
		return path.join(os.homedir(), "Documents", "CodinIT", "Rules") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return CodinITRulesDir
}

export async function ensureMcpServersDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const mcpServersDir = path.join(userDocumentsPath, "CodinIT", "MCP")
	try {
		await fs.mkdir(mcpServersDir, { recursive: true })
	} catch (error) {
		return "~/Documents/Cody/MCP" // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
	}
...
export async function getSavedCodinITMessages(context: vscode.ExtensionContext, taskId: string): Promise<CodinITMessage[]> {
...
export async function saveCodinITMessages(context: vscode.ExtensionContext, taskId: string, uiMessages: CodinITMessage[]) {
	try {
