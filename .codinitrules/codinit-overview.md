# CodinIT Extension Architecture & Development Guide

## Project Overview

CodinIT is a VSCode extension that provides AI assistance through a combination of a core extension backend and a React-based webview frontend. The extension is built with TypeScript and follows a modular architecture pattern.
...
The WebviewProvider maintains a reference to the Controller and delegates message handling to it. It also handles the creation of both sidebar and tab panel webviews, allowing CodinIT to be used in different contexts within VSCode.
...
CodinIT supports multiple AI providers through a modular API provider system. Each provider is implemented as a separate module in the `src/api/providers/` directory and follows a common interface.
...
CodinIT supports separate model configurations for Plan and Act modes:
...
class Task {
  async initiateTaskLoop(userContent: UserContent, isNewTask: boolean) {
    while (!this.abort) {
      ...
      const recDidEndLoop = await this.recursivelyMakeCodinITRequests(
...
class Task {
  async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
    ...
    const previousRequest = this.CodinITMessages[previousApiReqIndex]
...
CodinIT implements a dual-mode system that separates planning from execution:
...
CodinIT supports two types of MCP server connections:
...
class Controller {
  ...
  async downloadMcp(mcpId: string) {
    ...
    await this.initCodinITWithTask(task)
  }
}
```
...
This guide provides a comprehensive overview of the CodinIT extension architecture, with special focus on state management, data persistence, and code organization. Following these patterns ensures robust feature implementation with proper state handling across the extension's components.
...
Contributions to the CodinIT extension are welcome! Please follow these guidelines:
...
The `.CodinITignore` file allows users to specify files and directories that CodinIT should not access. When implementing new features, respect the `.CodinITignore` rules and ensure that your code does not attempt to read or modify ignored files.
