...
export async function POST(req: Request) {
  ...
  return tracer.startActiveSpan("chat.POST", async (span) => {
    ...
    const langfuseGeneration = langfuseTrace.generation({
      ...
      model: model.id,
      modelParameters: cleanProviderSpecificConfig,
      input: messages,
    });
    ...
    try {
      ...
      // Use toTextStreamResponse directly and capture the response for Langfuse
      const response = stream.toTextStreamResponse();
      console.log(`[Chat API ${requestId}] Stream created successfully`);

      // Clone the response to read its body for Langfuse logging
      const clonedResponse = response.clone();
      clonedResponse.text().then((text) => {
        langfuseGeneration.end({ output: text });
      }).catch((error) => {
        langfuseGeneration.end({ output: `Error reading response for Langfuse: ${error.message}`, level: "ERROR" });
      });

      return response;
    } catch (error: any) {
      langfuseGeneration.end({ output: error.message, level: "ERROR" });
