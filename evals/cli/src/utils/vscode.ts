...
/**
 * Spawn a VSCode instance with the CodinIT extension
...
export async function spawnVSCode(workspacePath: string, vsixPath?: string): Promise<VSCodeResources> {
	...
	if (!vsixPath) {
		try {
			...
			const CodinITRoot = path.resolve(process.cwd(), "..", "..")
	...
	const tempUserDataDir = path.join(os.tmpdir(), `vscode-CodinIT-eval-${Date.now()}`)
	...
	const tempExtensionsDir = path.join(os.tmpdir(), `vscode-CodinIT-eval-ext-${Date.now()}`)
	...
	fs.writeFileSync(
		evalsEnvPath,
		`# This file activates CodinIT test mode
# Created at: ${new Date().toISOString()}
# 
# This file is automatically detected by the CodinIT extension
...
`,
	...
	// and configure CodinIT to auto-open on startup
	...
	const settings = {
		...
		// Auto-open CodinIT on startup
		"CodinIT.autoOpenOnStartup": true,
	...
	console.log(`Created settings.json to disable workspace trust and auto-open CodinIT`)

	// Create keybindings.json to automatically open CodinIT on startup
	...
	const keybindings = [
		...
		{
			key: "alt+shift+c",
			command: "CodinIT.openInNewTab",
			when: "viewContainer.workbench.view.extension.saoudrizwan.claude-dev-ActivityBar.enabled",
		},
	]
	fs.writeFileSync(keybindingsPath, JSON.stringify(keybindings, null, 2))
	console.log(`Created keybindings.json to help with CodinIT activation`)
	...
	const args = [
		...
		// Run a command on startup to open CodinIT
		"--command",
	...
	const startupScript = `
		...
		setTimeout(() => {
			// Try to open CodinIT in the sidebar
			require('vscode').commands.executeCommand('workbench.view.extension.saoudrizwan.claude-dev-ActivityBar');
			
			// Also try to open CodinIT in a tab as a fallback
			setTimeout(() => {
				require('vscode').commands.executeCommand('CodinIT.openInNewTab');
			}, 5000);
		}, 5000);
	`
	fs.writeFileSync(startupScriptPath, startupScript)
	console.log(`Created startup script to activate CodinIT`)
	...
	try {
		...
		const extensionDir = path.join(tempExtensionsDir, "CodinIT-activator")
		fs.mkdirSync(extensionDir, { recursive: true })

		// Create package.json for the extension
		const packageJsonPath = path.join(extensionDir, "package.json")
		const packageJson = {
			name: "CodinIT-activator",
			displayName: "CodinIT Activator",
			description: "Activates CodinIT and starts the test server",
			version: "0.0.1",
			engines: {
				vscode: "^1.60.0",
			},
			...
			contributes: {
				commands: [
					{
						command: "CodinIT-activator.activate",
						title: "Activate CodinIT",
					},
				],
			},
		}
		...
		const extensionJs = `
			...
			function activate(context) {
				console.log('CodinIT Activator is now active!');
				
				// Register the command to activate CodinIT
				let disposable = vscode.commands.registerCommand('CodinIT-activator.activate', async function () {
					try {
						// Make sure the CodinIT extension is activated
						const extension = vscode.extensions.getExtension('saoudrizwan.claude-dev');
						if (!extension) {
							console.error('CodinIT extension not found');
							return;
						}
						
						if (!extension.isActive) {
							console.log('Activating CodinIT extension...');
							await extension.activate();
						}
						
						// Show the CodinIT sidebar
						console.log('Opening CodinIT sidebar...');
						...
						// Also open CodinIT in a tab as a fallback
						console.log('Opening CodinIT in a tab...');
						await vscode.commands.executeCommand('CodinIT.openInNewTab');
						...
						const CodinITRootPath = '${path.resolve(process.cwd(), "..", "..")}';
						const visibleWebview = require(path.join(CodinITRootPath, 'src', 'core', 'webview')).WebviewProvider.getVisibleInstance();
						if (visibleWebview) {
							require(path.join(CodinITRootPath, 'src', 'services', 'test', 'TestServer')).createTestServer(visibleWebview);
							console.log('Test server created successfully');
						} else {
							console.error('No visible webview instance found');
						}
					} catch (error) {
						console.error('Error activating CodinIT:', error);
					}
				});
				...
				setTimeout(() => {
					vscode.commands.executeCommand('CodinIT-activator.activate');
				}, 5000);
			}
		...
		console.log(`Created CodinIT Activator extension`)

		// Try multiple approaches to activate the extension
		let serverStarted = false

		// Create an activation script to run in VS Code
		const activationScriptPath = path.join(settingsDir, "activate-CodinIT.js")
		const activationScript = `
			// This script will be executed to activate CodinIT and start the test server
			const vscode = require('vscode');
			
			// Execute the CodinIT-activator.activate command
			vscode.commands.executeCommand('CodinIT-activator.activate');
		`
		...
		try {
			console.log("Executing activation script to start CodinIT and test server...")
			await execa(
				"code",
				[
					"--user-data-dir",
		...
		if (!serverStarted) {
			...
			console.log("You may need to manually open the CodinIT extension in VS Code")
		}
